...
export default () => {
    ...
    const loadUsers = async () => {
        ...
        const result = await axios.get(`https://crm-09.herokuapp.com/task/taskAuths?date=${date}&st=rejected`, headers);
                        ...
                        <div className='auth-box mt-5 '>
                            <div className="content">
                                <div className="yesterday-task">
                                    <div><h4>Today Rejected Tasks Detail</h4></div>
                                     <div className="action-btn">
                                    <button type="submit" className="btn btn-danger" onClick={()=>Navigate('/admin/rejectedtasks/monthly')}>Monthly Rejected Tasks</button>
                                    </div>
                                </div>

                                {
                                    users.map((user) => (
                                        <form>
                                            ...
                                            <div className="form-group mt-4">
                                                <label className="labels">Rejection Reason : {user.taskReject}</label>
                                            </div>
                                            <div className="form-group mt-4">
