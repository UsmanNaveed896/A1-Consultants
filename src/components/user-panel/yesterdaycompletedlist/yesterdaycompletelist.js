...
export default () => {
    ...
    const loadUsers = async () => {
        ...
        const result = await axios.get(`https://crm-09.herokuapp.com/task/taskcomp?date=${yesterdaydate}`, headers);
                    ...
                    <div className="  vertical-align-middle">
                        <div className='auth-box mt-5 '>
                            <div className="content">
                                <h3>Yesterday Completed Tasks List</h3>
                                {loader ?  <div>  
                                {
                                    users.length===0 ? <div className="alert-msg mt-4"><h5>No Records Today</h5><i class="fa fa-times" aria-hidden="true"></i></div> :
                                    users.map((user) => (
                                        <div className="employee-data" onClick={() => (Navigate(`/yesterdaycompleted/detail/${user._id}`))}>
